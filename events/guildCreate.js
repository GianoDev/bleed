...
client.on("guildCreate", guild => {
  ...
  if (guild.memberCount < 30) return channelToSend.send({ embed: { color: "efa23a", description: `${warn} Guild is below **30** members (\`Leaving\`)` } }).then(() => guild.leave())
  else channelToSend.send(embed)
})