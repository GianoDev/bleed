...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!log) return message.channel.send({ embed: { color: "", description: `${warn} ${message.author}: Modlogs channel was **not found** - set it using \`${prefix}modlogs (channel)\`` } })
    var mod = message.author
    var time = args[1]
    let rson = args.slice(2).join(' ')

    if (!user) return message.reply('You didn\'t mention a user')
    if (!time) return message.reply('Please write a time, Ex : 1s/1m/1h/1d/1w')
    if (!rson) return message.reply('You didn\'t say a reason!')
    ...
    setTimeout(function () {
      ...
      message.channel.send(`<@${user.id}> Now is not a jailed.`)
    }, ms(mutetime));
